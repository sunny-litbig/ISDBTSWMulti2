...
static uint8_t *load_be_uint32(uint32_t *dst, uint8_t *src)
{
	*dst = ((src[0]<<24)|(src[1]<<16)|(src[2]<<8)|src[3]);
	return src+4;
}

static uint8_t *save_be_uint32(uint8_t *dst, uint32_t src)
{
...
static uint32_t left_rotate_uint32(uint32_t val, uint32_t count)
{
...
static int set_scramble_key_multi2_with_parity(void *m2, uint8_t parity, uint8_t *val);
...
MULTI2 *create_multi2(void)
{
	...
	r->set_scramble_key = set_scramble_key_multi2_with_parity;//set_scramble_key_multi2;
...
static int set_round_multi2(void *m2, int32_t val)
{
	...
	prv = private_data(m2);
	if(prv == NULL){
		MPRINTF("[%s] MULTI2_ERROR_INVALID_PARAMETER\n", __func__);
		/* do nothing */
...
static int set_scramble_key_multi2_with_parity(void *m2, uint8_t parity, uint8_t *val)
{
	...
	p = load_be_uint32(&(prv->scr[parity].l), p);
	p = load_be_uint32(&(prv->scr[parity].r), p);
	
	core_schedule(prv->wrk+parity, &(prv->sys), prv->scr+parity);
	...
	if(parity){
		...
		prv->state |= MULTI2_STATE_SCRAMBLE_KEY_SET2;
	}
#else
	prv->state |= MULTI2_STATE_SCRAMBLE_KEY_SET;
#endif /* 0 */
...
static int decrypt_multi2(void *m2, int32_t type, uint8_t *buf, int32_t size)
{
	...
	if(prv->state != (MULTI2_STATE_CBC_INIT_SET|MULTI2_STATE_SYSTEM_KEY_SET|MULTI2_STATE_SCRAMBLE_KEY_SET)){
		if( (prv->state & MULTI2_STATE_CBC_INIT_SET) == 0 ){
			MPRINTF("[%s] MULTI2_ERROR_UNSET_CBC_INIT\n", __func__);
			return MULTI2_ERROR_UNSET_CBC_INIT;
		}
		if( (prv->state & MULTI2_STATE_SYSTEM_KEY_SET) == 0 ){
			MPRINTF("[%s] MULTI2_ERROR_UNSET_SYSTEM_KEY\n", __func__);
			return MULTI2_ERROR_UNSET_SYSTEM_KEY;
		}
		if( (prv->state & MULTI2_STATE_SCRAMBLE_KEY_SET) == 0 ){
			MPRINTF("[%s] MULTI2_ERROR_UNSET_SCRAMBLE_KEY\n", __func__);
			return MULTI2_ERROR_UNSET_SCRAMBLE_KEY;
		}
	#if 0
		if( (prv->state & MULTI2_STATE_SCRAMBLE_KEY_SET2) == 0 ){
			MPRINTF("[%s] MULTI2_ERROR_UNSET_SCRAMBLE_KEY2\n", __func__);
			return MULTI2_ERROR_UNSET_SCRAMBLE_KEY;
		}
	#endif /* 0 */
	}
	...
	MPRINTF("[%s] cbc(0x%x, 0x%x)\n", __func__, cbc.l, cbc.r);
